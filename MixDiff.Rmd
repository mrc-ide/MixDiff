...
2. More flexibility in the proposal for the different delays and in acceptance rate recording (both)
3. Adaptive proposal? (both)
4. Speed up using profiling (both)
5. Compute index_dates_order AUTOMATICALLY FROM index_dates (Anne)
6. Write some code to restart from last point of the chain (Anne)
7. Where we use ncol(curr_aug_dat$D[[g]]), check this as I think it may need to be defined from index_dates rather than from D (Anne)
8. do we indeed want to update zeta after each D_i move? maybe not useful? (both -> but Marc to look at run MCMC)
9. wait for Rich to generate an R package on reading the MNIST data, this way we can call this pakcage rather than have the functions copied in our code directly (both)
