...
with 50% threshold for posterior support for error) for which the true date has 
non zero posterior probability 
2) proportion of missing dates (respectively erroneous dates identified as such 
...
3) correlation coefficient / R2 / other measures of the residual between the
mode of the posterior augmented date and the true date
4) log-score: average logposterior probability of the true date (close to 0 
means the model is doing a great job) - issue if model is not doing well for 
some dates --> there will be some -inf values...
