...
* Small amendments of current code
1. Change hyperprior to hyperparameters everywhere (Anne)
2. Explorer simulations en parallele (Anne)
3. More flexibility in the proposal for the different delays and in acceptance rate recording (both)
4. Adaptive proposal? (both)
5. Speed up using profiling (both)
6. Compute index_dates_order AUTOMATICALLY FROM index_dates (Anne)
7. Write some code to restart from last point of the chain (Anne)
8. Where we use ncol(curr_aug_dat$D[[g]]), check this as I think it may need to be defined from index_dates rather than from D (Anne)
9. do we indeed want to update zeta after each D_i move? maybe not useful? (both -> but Marc to look at run MCMC)
10. wait for Rich to generate an R package on reading the MNIST data, this way we can call this pakcage rather than have the functions copied in our code directly (both)

* More substantial changes
1. Mettre sur papier le model d'erreur (both & today)
2. Design (sur papier) l'interface pour collecte de data error (both & today)
3. Voir pour l'hebergement (Anne)
4. Code pour le shiny app (Marc)
...
3. Think about alternative (Marc)
