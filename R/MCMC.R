      ...
      for (g in seq_len(n_groups)) {
        ...
        for (j in seq_len(ncol(curr_aug_dat$D[[g]]))) {
          ...
          for (i in seq_len(n_groups_to_update)) {
            # print(i)
            tmp <- move_Di(
              to_update[MCMC_settings$moves_options$move_D_by_groups_of_size * (i - 1) + (seq_len(MCMC_settings$moves_options$move_D_by_groups_of_size))], g, j,
              curr_aug_dat,
              curr_theta,
              obs_dat,
              hyperparameters,
    ...
    if (MCMC_settings$moves_switch$mu_on) {
      for (g in seq_len(n_groups)) {
        ...
        for (j in seq(2, ncol(curr_aug_dat$D[[g]]), 1)) {
          # print(j)
          tmp <- move_lognormal(
            what = "mu", g, j - 1, MCMC_settings$moves_options$sdlog_mu[[g]][[j - 1]],
            curr_aug_dat,
            curr_theta,
            obs_dat,
            hyperparameters,
    ...
    if (MCMC_settings$moves_switch$CV_on) {
      for (g in seq_len(n_groups)) {
        ...
        for (j in seq(2, ncol(curr_aug_dat$D[[g]]), 1))
        {
          # print(j)
          tmp <- move_lognormal(
            what = "CV", g, j - 1, MCMC_settings$moves_options$sdlog_CV[[g]][[j - 1]],
            curr_aug_dat,
            curr_theta,
            obs_dat,
            hyperparameters,
