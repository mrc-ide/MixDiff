...
move_Di <- function(i,
  ...
  from_value <- sapply(
    seq_len(nrow(x)),
    function(k) {
      group_data <- curr_aug_dat$D[[group_idx]]
      row <- i
      cols <- index_dates[[group_idx]][-x[k, 1], x[k, 2]]
      group_data[row, cols]
    }
  )
  ...
  erroneous <- which(
    !is.na(obs_dat[[group_idx]][i, date_idx]) &
      proposed_aug_dat$D[[group_idx]][i, date_idx] != obs_dat[[group_idx]][i, date_idx]
  )
  ...
  # only need to look at the error term if some of the E have changed
  if (any(different_E)) {
...
swap_Ei <- function(i,
  ...
  ## LL_error_term_slow(proposed_aug_dat, theta, obs_dat) -
  ...
  logcorrection_move_from_E1_to_E0 <- sum(
    sapply(seq_along(date_idx_E1_to_E0), function (e) {
