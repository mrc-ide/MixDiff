...
calculate_date_matrix <- function(start_date, end_date, p_error, log = TRUE)
{
  ...
  if(log) likelihood_error_date <- log(likelihood_error_date)
...
dErrorDate <- function(date1, date2, date_space, date_transition){
  d1 <- which(date_space==date1) # underlying date we are conditionning on
  d2 <- which(date_space==date2) # observed date
